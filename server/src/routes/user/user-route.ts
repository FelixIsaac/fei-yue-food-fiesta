import { FastifyInstance, FastifyPluginOptions, FastifyError, FastifyRequest, RequestBodyDefault } from "fastify";
...
export default ((server: FastifyInstance, options: FastifyPluginOptions, next: (error?: FastifyError) => void) => {
    server.post("/", registerUser, async (request, reply) => {
        const response = await userController.createUser(request.body as Partial<IUser>);
